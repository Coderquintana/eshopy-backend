...
{
  public Task<KeycloakUserProvisioningResult> CreateUserAsync(string email, string name, string subdomain, TenantUserRole role, CancellationToken ct)
    => Task.FromResult(new KeycloakUserProvisioningResult($"fake-keycloak-user-{Guid.NewGuid():N}", $"fake-{Guid.NewGuid():N}"));
}
