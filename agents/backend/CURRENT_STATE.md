...
| **Store** | ✅ F-13A completado | `GET/PUT /api/store` expone marca y contacto. Logo y portada tienen subida real mediante `POST/DELETE /api/store/images/{logo|hero}`, procesamiento WebP y limpieza segura por Store; `StoreTheme` mantiene los knobs cosméticos en `Data`. `CurrencyCode` sigue inmutable tras creación |
