...
| # | Tarea | Descripcion |
|---|---|---|
| F5-02 | CurrencyCode elegido en el onboarding, no hardcodeado | Ver decisión completa en `GOVERNANCE.md` ("Moneda del tenant: configurable, no atada a PYG"). Resumen: `CreateTenantCommand` gana `CurrencyCode` requerido (formato ISO 4217, 3 letras, validado por forma — no contra una lista fija de países); `Store.CreateDefault` lo recibe como parámetro en vez del literal `"PYG"`. No toca `PlanPricing.cs` (esa es la moneda con la que vos le cobrás el plan al tenant, un eje distinto — sigue como estaba, ya trackeado en F4-06). No requiere cambios de frontend: el onboarding sigue siendo manual por Postman hasta que exista L-04 |
