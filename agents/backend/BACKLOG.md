...
| INFRA-01 | Unificar la persistencia de Keycloak sobre SQL Server | Pendiente, no implementar todavía. Eliminar el contenedor PostgreSQL `keycloak-db` y conectar Keycloak al `eshopy-sqlserver` existente mediante `KC_DB=mssql`, conservando una base `keycloak` y credenciales propias separadas de la base de eShopy. Crear la base con collation UTF-8 y `READ_COMMITTED_SNAPSHOT`; actualizar Compose y la guía de setup. Los datos actuales de PostgreSQL no migran automáticamente: en desarrollo se recrea/importa el realm y antes de cualquier entorno persistente se define una migración o respaldo explícito |
...
| C-68 | **Bug real (encontrado en vivo desde el Admin, 2026-09-12, cargando productos en una tienda nueva)**: publicar cualquier producto fallaba siempre con un 400 genérico ("Algo no salió como esperaba"). Causa: `ChangeProductStatusCommand.Status`/`ChangeOrderStatusCommand.Status` son el enum de C# (`ProductStatus`/`OrderStatus`) sin `[JsonConverter]`, y no hay ningún `JsonStringEnumConverter` registrado — `System.Text.Json` sin converter exige el valor numérico del enum al deserializar. El Admin manda siempre el nombre (`"Active"`), como cualquier cliente HTTP que no conoce el tipo C#, así que el bind fallaba antes de llegar al handler, con el 400 nativo de ASP.NET (`ValidationProblemDetails`, sin el `code` del `ErrorResponse` propio) — el frontend lo mostraba como error genérico por no reconocer ese cuerpo. Invisible en los tests existentes porque construyen el comando en C# y lo pasan a `JsonContent.Create(command)`, que serializa el enum como número (`1`) sin converter — nunca ejercitaron la forma real que manda el navegador. Fix: `JsonStringEnumConverter` global en `Program.cs` (mismo `AddJsonOptions` que ya tiene los converters de fecha de D-05), con `allowIntegerValues` en su default `true` — acepta string y número al leer, así los tests viejos (numéricos) siguen pasando sin tocarlos. Mismo bug afectaba `PATCH /api/orders/{id}/status` (cancelar/reembolsar desde el Admin), resuelto por el mismo converter global sin cambios adicionales. Test nuevo (`ChangeStatus_WithStringEnumBody_ShouldSucceed`) manda el `PATCH` con el mismo shape exacto que el Admin real (`{"status":"Active","rowVersion":"..."}`) para que este bug no pueda volver en silencio | Catalog/Orders | 2026-09-12 |
