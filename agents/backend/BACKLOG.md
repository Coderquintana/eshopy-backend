...
| F4-09 | Campos nuevos de Store — identidad de negocio + knobs de `StoreTheme` (catalogo cerrado, 2026-09-11) | Definido y cerrado 2026-09-11 — instruccion completa, no inventar ni ampliar alcance sin otra sesion de definicion. Hoy `Store` tiene 6 campos (`Name`, `Timezone`, `PrimaryColor`, `LogoUrl`, `BackgroundColor`, `Description`); el catalogo final agrega 10 mas, **16 en total**, separados en dos grupos con tratamiento distinto (ver GOVERNANCE.md "Personalizacion cosmetica de Store"): <br><br>**Columnas reales nuevas** (identidad de negocio, con su propia validacion de dominio, igual que las 6 actuales — NO van al `Data` JSON): `ContactWhatsapp` (string?, formato de telefono), `ContactEmail` (string?, formato email, distinto del email del Owner en Keycloak), `InstagramUrl` (string?, URL absoluta), `FacebookUrl` (string?, URL absoluta), `Address` (string?, texto libre, limite razonable), `BusinessHours` (string?, texto libre por ahora — un modelo estructurado dia/hora es sobre-ingenieria para esta etapa). Estos dos ultimos (redes sociales, horario de atencion) ya estaban anotados como "afuera a proposito" en F-09 del frontend — esta tarea cierra ese pendiente. <br><br>**Knobs de `StoreTheme`** (van al `Data` JSON via `SetData<StoreTheme>`, sin migracion nueva por cada uno): `FontFamily` (string?, de una lista cerrada de fuentes permitidas, no cualquier valor arbitrario — no choca con L-06 self-host de fuentes del frontend cuando se implemente), `HeadingScale` (string?, enum chico de preajustes — "Compacta"/"Normal"/"Grande" — no un valor libre en px, mapea a la escala tipografica existente), `BorderRadius` (string?, enum chico — "Cuadrado"/"Redondeado" — mapea a `--radius-*`), `SpacingDensity` (string?, enum chico — "Compacta"/"Normal"/"Amplia" — mapea a `--space-*`). <br><br>**Deliberadamente descartado en esta pasada, con motivo tecnico**: video o imagen de fondo por tenant. No es un valor, es un archivo — mismo problema no resuelto que F5-04 (storage, ancho de banda, sin volumen persistente todavia), y un fondo en video ademas choca de frente con la regla de `_motion.scss` contra animacion constante (riesgo real de mareo vestibular, no solo gusto). No reconsiderar hasta que F5-04 este cerrado. <br><br>Migracion nueva solo para las 6 columnas reales, no para `StoreTheme`. Actualizar `UpdateStoreCommand`, el DTO de lectura del Admin, validadores, y el `Documentation/Postman/`. Ver tambien `eshopy-frontend/agents/BACKLOG.md` F-12, que depende de esto |
