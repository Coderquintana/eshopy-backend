...
| F4-09 | Campos nuevos de Store — identidad de negocio + primeros knobs de `StoreTheme` | Definido 2026-09-11, para el otro agente que lo implemente — instruccion completa, no inventar alcance. Hoy `Store` tiene 6 campos (`Name`, `Timezone`, `PrimaryColor`, `LogoUrl`, `BackgroundColor`, `Description`); agregar los siguientes, **separados en dos grupos con tratamiento distinto** (ver GOVERNANCE.md "Personalizacion cosmetica de Store"): <br><br>**Columnas reales nuevas** (identidad de negocio, con su propia validacion de dominio, igual que las 6 actuales — NO van al `Data` JSON): `ContactWhatsapp` (string?, formato de telefono), `ContactEmail` (string?, formato email, distinto del email del Owner en Keycloak), `InstagramUrl` (string?, URL absoluta), `FacebookUrl` (string?, URL absoluta), `Address` (string?, texto libre, limite razonable), `BusinessHours` (string?, texto libre por ahora — un modelo estructurado dia/hora es sobre-ingenieria para esta etapa). Estos dos ultimos (redes sociales, horario de atencion) ya estaban anotados como "afuera a proposito" en F-09 del frontend — esta tarea cierra ese pendiente. <br><br>**Primeros knobs de `StoreTheme`** (van al `Data` JSON via `SetData<StoreTheme>`, sin migracion nueva por cada uno — activar el patron que hoy existe pero `Store` no usa): `FontFamily` (string?, de una lista cerrada de fuentes permitidas — no cualquier valor arbitrario, para no chocar con L-06 self-host de fuentes del frontend cuando se implemente). <br><br>Esta lista es un piso razonable para la primera pasada, no el techo — no es exhaustiva a proposito, se puede ampliar en otra sesion de definicion. Migracion nueva solo para las columnas reales, no para `StoreTheme`. Actualizar `UpdateStoreCommand`, el DTO de lectura del Admin, validadores, y el `Documentation/Postman/`. Ver tambien `eshopy-frontend/agents/BACKLOG.md` F-12, que depende de esto |
