...
> Estado al 2026-09-06 (F8-07, ver C-53). Antes de eso, al 2026-07-26. Reauditado contra el codigo real en HEAD (d531917, ultimo commit 2026-02-20) tras una pausa de ~5 meses.
...
| D-05 | `DateTime` sin `Kind=Utc` en las respuestas JSON | Core | Las fechas se guardan en `datetime2` (sin offset) y vuelven de EF con `DateTimeKind.Unspecified`, asi que `System.Text.Json` las serializa sin sufijo `Z` (`"2026-09-06T18:51:43.8144006"`). El frontend en JS las interpreta como **hora local**, no UTC — hoy son 4h de diferencia en Paraguay. Detectado el 2026-09-06 verificando F8-07 en vivo; afecta a todos los DTOs con fechas (`OrderAdminDto`, `PublicOrderDto`, `ProductAdminDto`, …), no solo a los nuevos. Contradice la convencion "Fechas ISO 8601 UTC siempre" de GOVERNANCE.md. Fix candidato: un `JsonConverter<DateTime>` global que fuerce `DateTimeKind.Utc` al serializar (no tocar cada DTO). **Pendiente, no implementado** |
